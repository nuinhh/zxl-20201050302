...
void init() {			//光源、材质、真实感初始化
	...
	glMaterialfv(GL_FRONT, GL_DIFFUSE, diffuse);		//设置材料反射指数
	//设置光源位置
	glLightfv(GL_LIGHT0, GL_POSITION, position);
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT,ambient);
	glLightModelfv(GL_LIGHT_MODEL_LOCAL_VIEWER, local_view);
	glLightModelfv(GL_LIGHT_MODEL_TWO_SIDE, local_view);
	glMaterialfv(GL_FRONT, GL_EMISSION, matEmission);  // 用来设置物体发光效果
...
void setMenu()			//设置菜单
{
	...
	glutAddMenuEntry("球体", 2);
	glutAddMenuEntry("茶壶", 3);

	slrMenu = glutCreateMenu(menuFunc);
	glutAddMenuEntry("线框", 4);
	glutAddMenuEntry("实体", 5);

	colorMenu = glutCreateMenu(menuFunc);
	glutAddMenuEntry("红色", 6);
	glutAddMenuEntry("品红色", 7);
	glutAddMenuEntry("绿色", 8);
	glutAddMenuEntry("蓝色", 9);
	glutAddMenuEntry("白色", 10);
	glutAddMenuEntry("黑色", 11);

	lightMenu = glutCreateMenu(menuFunc);
	glutAddMenuEntry("启用", 12);
	glutAddMenuEntry("关闭", 13);

	lightPositionMenu = glutCreateMenu(menuFunc);
	glutAddMenuEntry("光源向下移动", 14);
	glutAddMenuEntry("光源向上移动", 15);
	glutAddMenuEntry("光源向外移动", 16);
	glutAddMenuEntry("光源向内移动", 17);
